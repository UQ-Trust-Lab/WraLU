...
def milp_callback(model, where):
    if where == GRB.Callback.MIP:
        ...
        if obj_bound > 0.01 or obj_best < -0.01:
            model.terminate()
...
def lp_callback(model, where):
    if where == GRB.Callback.SIMPLEX:
        ...
        if model.cbGet(GRB.Callback.SPX_PRIMINF) == 0 and obj_best < -0.00001:
            # and model.cbGet(GRB.Callback.SPX_DUALINF) == 0:
            model.terminate()
    if where == GRB.Callback.BARRIER:
        obj_best = model.cbGet(GRB.Callback.SPX_OBJVAL)
        if model.cbGet(GRB.Callback.BARRIER_PRIMINF) == 0 and obj_best < -0.00001:
...
def _add_kactivation_constraints(model, var_list, constraint_groups, x_counter, y_counter, kact_constrs):
    for h, inst in enumerate(constraint_groups):
        for i, row in enumerate(inst.cons):
            k = len(inst.varsid)
            expr = LinExpr()
            expr.addConstant(row[0])
            for j, x in enumerate(inst.varsid):
                expr.addTerms(row[1 + j], var_list[x_counter + x])
                expr.addTerms(row[1 + k + j], var_list[y_counter + x])
            constr = model.addConstr(expr >= 0., name=f"kact_{x_counter}_{y_counter}_{h}_{i}")
            kact_constrs.append(constr)

def handle_relu(model, var_list, affine_counter, num_neurons, lbi, ubi, relu_group, use_milp, partial_milp_neurons,
                kact_constrs):
    use_milp = use_milp and config.use_milp
    # print("relu groups ")
    ...
    if len(relax_encode_idx) > 0:
        ...
        for j in relax_encode_idx:
            if ubi[j] <= 0:
                ...
                expr = var_list[relu_counter + j] - var_list[affine_counter + j]
                model.addConstr(expr, GRB.EQUAL, 0)
            # elif lbi[j] < -0.01 and ubi[j] > 0.01:
            elif lbi[j] < 0 < ubi[j]:
                # y >= x
                expr = var_list[relu_counter + j] - var_list[affine_counter + j]
                model.addConstr(expr, GRB.GREATER_EQUAL, 0)
                ...
                expr += lbi[j] * ubi[j]
                model.addConstr(expr, GRB.LESS_EQUAL, 0, name="relu_triangle_" + str(j))

    if len(relu_group) > 0:
        _add_kactivation_constraints(model, var_list, relu_group, affine_counter, relu_counter, kact_constrs)
...
def handle_tanh_sigmoid(model, var_list, affine_counter, num_neurons, lbi, ubi,
                        constraint_groups, activation_type, kact_constrs):
    ...
    _add_kactivation_constraints(model, var_list, constraint_groups, affine_counter, y_counter, kact_constrs)
...
def create_model(nn, LB_N0, UB_N0, nlb, nub, relu_groups, numlayer, use_milp, is_nchw=False, partial_milp=0,
        ...
        if nn.layertypes[i] in ['SkipCat']:
            ...
            if relu_groups is None:
                counter = handle_relu(model, var_list, counter, len(nlb[i]), nlb[index - 1], nub[index - 1], [],
                                      use_milp, partial_milp_neurons, kact_constrs)
            else:
                counter = handle_relu(model, var_list, counter, len(nlb[i]), nlb[index - 1], nub[index - 1],
                                      relu_groups[activation_counter], use_milp, partial_milp_neurons, kact_constrs)
            ...
            if relu_groups is None:
                counter = handle_tanh_sigmoid(model, var_list, counter, len(nlb[i]), nlb[index - 1], nub[index - 1],
                                              [], nn.layertypes[i], kact_constrs)
            else:
                counter = handle_tanh_sigmoid(model, var_list, counter, len(nlb[i]), nlb[index - 1], nub[index - 1],
                                              relu_groups[activation_counter], nn.layertypes[i], kact_constrs)
            activation_counter += 1
            start_counter.append(counter)
    ...
    return counter, var_list, model, kact_constrs
