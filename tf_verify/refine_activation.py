...
def refine_activation_with_solver_bounds(nn, self, man, element, nlb, nub, relu_groups, timeout_lp, timeout_milp,
                                         ...
                                         K=3, s=-2, use_milp=False, approx=config.approx_k):
    """
    ...
    if nn.activation_counter == 0:
        if domain == 'deepzono':
            encode_kactivation_cons(nn, man, element, offset, predecessor_index, length, lbi, ubi,
                                    relu_groups, False, 'refinezono', nn.layertypes[layerno])
        ...
        if config.refine_neurons and nn.layertypes[predecessor_index] == "FC":
            if 0 < sum(
                    (affine_layers >= second_FC).__and__(predecessor_index >= affine_layers)) <= config.n_milp_refine:
                # predecessor_index >= second_FC :#and domain=="deepzono" and predecessor_index>second_FC:
            ...
            nlb[predecessor_index], nub[predecessor_index] = resl, resu

        lbi, ubi = nlb[predecessor_index], nub[predecessor_index]
